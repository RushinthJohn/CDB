...
execute 'Starting Config Server A' do
  command 'mongod --replSet cfgst --logpath "cfg-a.log" --dbpath /data/config/config-a --port 57020 --fork --configsvr'
...
execute 'Starting MongoD Shard-0 Replica-0' do
  command 'mongod --replSet s0 --logpath "s0-r0.log" --dbpath /data/shard0/rs0 --port 37020 --fork --shardsvr'
  #not_if 'pstree | grep mongod'
...
execute 'Starting MongoD Shard-1 Replica-0' do
  command 'mongod --replSet s1 --logpath "s1-r0.log" --dbpath /data/shard1/rs0 --port 47020 --fork --shardsvr'
  #not_if 'pstree | grep mongod'
  #action :nothing
end
